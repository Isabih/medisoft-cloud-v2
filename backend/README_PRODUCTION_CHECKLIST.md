...
2. Keep FastAPI bound to `127.0.0.1:8000` and use nginx as the public `/api/` entrypoint.
3. Use `API_BASE_URL=http://YOUR_SERVER_IP_OR_DOMAIN` in agents, not `:8000`.
...
7. Put the backend behind a systemd service and verify nginx proxying to `127.0.0.1:8000`.
