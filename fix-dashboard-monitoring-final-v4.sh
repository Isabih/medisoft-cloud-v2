...
echo "=== Backup ==="
cp "$BACKEND/app/main.py" "$BACKEND/app/main.py.bak_final_$(date +%F_%H%M%S)"

echo "=== Remove conflicting old dashboard overrides ==="
python3 - <<'PY'
...
for name in [
    "monitoring_cards_fix_v4",
    "dashboard_live_repair_v4",
    "dashboard_live_override",
]:
    s = s.replace(f"import app.routers.{name} as {name}\n", "")
    s = s.replace(f"app.include_router({name}.router, prefix=settings.api_v1_prefix)\n", "")

p.write_text(s)
print("old overrides removed")
PY

echo "=== Install final live v4 override ==="
cat > "$BACKEND/app/routers/dashboard_live_final_v4.py" <<'PY'
import json
...
echo "=== Register final override before dashboard router ==="
python3 - <<'PY'
from pathlib import Path
p = Path("/opt/medisoft-guardian-v3/backend/app/main.py")
s = p.read_text()

if "dashboard_live_final_v4" not in s:
    s = s.replace(
        "import app.routers.dashboard as dashboard",
        "import app.routers.dashboard_live_final_v4 as dashboard_live_final_v4\nimport app.routers.dashboard as dashboard"
    )

if "app.include_router(dashboard_live_final_v4.router" not in s:
    s = s.replace(
...
echo "=== Test ==="
curl -s http://127.0.0.1:8004/api/v1/dashboard/centers-live | jq '.[] | {
  name,status,heartbeat_age_minutes,backend_connection,rows,db_health_score,drift_reason,success_rate,risk_score
}'
