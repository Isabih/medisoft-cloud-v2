...
def database_table_summary():
    ...
    try:
        ...
        for t in tables:
            ...
            rows = 0
            try:
                rows = int(query_value(mysql_conn(DB_NAME), f"SELECT COUNT(*) FROM `{name}`", default=0) or 0)
            except Exception:
                rows = int(t.get("approx_rows") or 0)
...
def payload():
    ...
    return {
        ...
        "local_table_count": database_table_count(),
        "local_table_summary_json": database_table_summary(),
